...
func InvoicePdf(invoiceId string) ([]byte, error) {
	...
	customFonts, err := repository.New().
		AddUTF8Font("PlayfairDisplay", fontstyle.Normal, GetStatic("fonts/PlayfairDisplay/PlayfairDisplay-Regular.ttf")).
		AddUTF8Font("PlayfairDisplay", fontstyle.Bold, GetStatic("fonts/PlayfairDisplay/PlayfairDisplay-Bold.ttf")).
		AddUTF8Font("PlayfairDisplay", fontstyle.Italic, GetStatic("fonts/PlayfairDisplay/PlayfairDisplay-Italic.ttf")).
		AddUTF8Font("PlayfairDisplay", fontstyle.BoldItalic, GetStatic("fonts/PlayfairDisplay/PlayfairDisplay-BoldItalic.ttf")).
		Load()
	if err != nil {
		return nil, err
	}
	...
	doc.AddRow(35,
		mImage.NewFromFileCol(4, GetStatic("img/logo.png"), props.Rect{
			Left:    0,
			Top:     0,
			Percent: 80,
		}),
	...
	doc.AddRow(5,
		col.New(7).Add(
			text.New("191186, Санкт-Петербург, Невский пр., д. 30, оф. 6.11"),
		),
		col.New(5).Add(
	...
	doc.AddRow(5,
		col.New(7).Add(
			text.New("Северо-Западный банк ПАО Сбербанк", props.Text{Style: fontstyle.Bold}),
		),
		col.New(5).Add(
